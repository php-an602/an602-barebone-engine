...
{
	global $db, $auth, $engine_dispatcher;
	...
	*/
	$vars = array('rowset');
	extract($engine_dispatcher->trigger_event('core.make_forum_select_modify_forum_list', compact($vars)));
...
{
	global $db, $config, $engine_container;
...
function get_forum_list($acl_list = 'f_list', $id_only = true, $postable_only = false, $no_cache = false)
{
	...
	*/
	$vars = array('rowset');
	extract($engine_dispatcher->trigger_event('core.get_forum_list_modify_data', compact($vars)));
...
function copy_forum_permissions($src_forum_id, $dest_forum_ids, $clear_dest_perms = true, $add_log = true)
{
	...
	if ($add_log)
	{
		$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_FORUM_COPIED_PERMISSIONS', false, array($src_forum_name, implode(', ', $dest_forum_names)));
	}
...
function move_topics($topic_ids, $forum_id, $auto_sync = true)
{
	...
	extract($engine_dispatcher->trigger_event('core.move_topics_before', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.move_topics_before_query', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.move_topics_after', compact($vars)));
...
function move_posts($post_ids, $topic_id, $auto_sync = true)
{
	...
	extract($engine_dispatcher->trigger_event('core.move_posts_before', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.move_posts_after', compact($vars)));
	...
	if ($auto_sync)
	{
		...
		extract($engine_dispatcher->trigger_event('core.move_posts_sync_after', compact($vars)));
	}
...
function delete_topics($where_type, $where_ids, $auto_sync = true, $post_count_sync = true, $call_delete_posts = true)
{
	...
	extract($engine_dispatcher->trigger_event('core.delete_topics_before_query', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.delete_topics_after_query', compact($vars)));
	...
	$engine_notifications->delete_notifications(array(
...
{
	global $db, $config, $engine_root_path, $phpEx, $auth, $user, $engine_container, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_posts_before', compact($vars)));
...
/*		Possible code for splitting post deletion
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_posts_in_transaction_before', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_posts_in_transaction', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_posts_after', compact($vars)));
	...
	$engine_notifications->delete_notifications($delete_notifications_types, $post_ids);
	...
	switch ($mode)
	{
		...
		case 'forum':
			...
			if (count($post_ids))
			{
				...
				*/
				$vars = ['sql_ary'];
				extract($engine_dispatcher->trigger_event('core.sync_forum_last_post_info_sql', compact($vars)));
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.sync_modify_forum_data', compact($vars)));
			unset($post_info);
		...
		case 'topic':
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.sync_topic_last_post_info_sql', compact($vars)));
			$result = $db->sql_query($db->sql_build_query('SELECT', $sql_ary));
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				*/
				...
				extract($engine_dispatcher->trigger_event('core.sync_modify_topic_data', compact($vars)));
			}
			$db->sql_freeresult($result);
...
function prune($forum_id, $prune_mode, $prune_date, $prune_flags = 0, $auto_sync = true, $prune_limit = 0)
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.prune_sql', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.prune_delete_before', compact($vars)));
...
function auto_prune($forum_id, $prune_mode, $prune_flags, $prune_days, $prune_freq, $log_prune = true)
{
	...
	if ($row)
	{
		...
		if ($log_prune)
		{
			$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_AUTO_PRUNE', false, [$row['forum_name']]);
		}
	}
...
function view_log($mode, &$log, &$log_count, $limit = 0, $offset = 0, $forum_id = 0, $topic_id = 0, $user_id = 0, $limit_days = 0, $sort_by = 'l.log_time DESC', $keywords = '')
{
	...
	$log = $engine_log->get_logs($mode, $count_logs, $limit, $offset, $forum_id, $topic_id, $user_id, $limit_days, $sort_by, $keywords);
	$log_count = $engine_log->get_log_count();

	return $engine_log->get_valid_offset();
}
...
{
	global $user, $phpEx, $engine_extension_manager;
