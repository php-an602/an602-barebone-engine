...
// 2. To disable composer autoloading, ENGINE_NO_COMPOSER_AUTOLOAD can be specified.
// 	  Additionally specify ENGINE_AUTOLOAD=/path/to/autoload.php in the
...
// 3. You can also set ENGINE_NO_COMPOSER_AUTOLOAD without setting ENGINE_AUTOLOAD.
...
//    larger program.
if (getenv('ENGINE_NO_COMPOSER_AUTOLOAD'))
{
	if (getenv('ENGINE_AUTOLOAD'))
	{
		require(getenv('ENGINE_AUTOLOAD'));
	}
}
else
{
	if (!file_exists($engine_root_path . 'vendor/autoload.php'))
	{
		trigger_error(
