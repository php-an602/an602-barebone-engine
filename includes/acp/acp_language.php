...
class acp_language
{
	...
	{
		global $config, $db, $user, $template, $engine_log, $engine_container;
		global $engine_root_path, $phpEx, $request, $engine_dispatcher;
		...
		switch ($action)
		{
			case 'update_details':
				...
				$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_LANGUAGE_PACK_UPDATED', false, array($sql_ary['lang_english_name']));
			...
			case 'details':
				...
				if ($lang_iso != $config['default_lang'])
				{
						...
						$iterator = new \RecursiveIteratorIterator(
							new \phpbb\recursive_dot_prefix_filter_iterator(
								new \RecursiveDirectoryIterator(
									$engine_root_path . 'language/' . $config['default_lang'] . '/',
					...
					foreach ($iterator as $file_info)
					{
						...
						if (file_exists($engine_root_path . 'language/' . $lang_iso . '/' . $relative_path))
						{
			...
			case 'delete':
				...
				if (confirm_box(true))
				{
					...
					$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_LANGUAGE_PACK_DELETED', false, array($row['lang_english_name']));
					...
					extract($engine_dispatcher->trigger_event('core.acp_language_after_delete', compact($vars)));
			...
			case 'install':
				...
				if (!$lang_iso || !file_exists("{$engine_root_path}language/$lang_iso/iso.txt"))
				{
					trigger_error($user->lang['LANGUAGE_PACK_NOT_EXIST'] . adm_back_link($this->u_action), E_USER_WARNING);
				}

				$file = file("{$engine_root_path}language/$lang_iso/iso.txt");
				...
				$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_LANGUAGE_PACK_INSTALLED', false, array($lang_pack['name']));
		...
		$language_helper = $engine_container->get('language.helper.language_file');
	...
	*/
	function compare_language_files($source_lang, $dest_lang, $file)
	{
		...
		$source_file = $engine_root_path . 'language/' . $source_lang . '/' . $file;
		$dest_file = $engine_root_path . 'language/' . $dest_lang . '/' . $file;
