...
class acp_icons
{
	...
	function main($id, $mode)
	{
		...
		global $request, $engine_container;
		...
		if ($action == 'edit' || $action == 'add' || $action == 'import')
		{
			...
			foreach ($imglist as $path => $img_ary)
			{
				...
				{
					$img_size = getimagesize($engine_root_path . $img_path . '/' . $path . $img);
					...
					else
					{
						...
						$xml_get = simplexml_load_file($engine_root_path . $img_path . '/' . $path . $img);
			...
			if ($dir = @opendir($engine_root_path . $img_path))
			{
				while (($file = readdir($dir)) !== false)
				{
					if (is_file($engine_root_path . $img_path . '/' . $file) && preg_match('#\.pak$#i', $file))
					{
						$_paks[] = $file;
					}
				}
		...
		switch ($action)
		{
			...
			case 'add':
				...
				foreach ($data as $img => $img_row)
				{
					$template->assign_block_vars('items', array(
						...
						'IMG_SRC'	=> $engine_root_path . $img_path . '/' . $img,
				...
				if ($action == 'add' && $mode == 'smilies' && count($smilies))
				{
					$template->assign_vars(array(
						...
						'IMG_SRC'			=> $engine_root_path . $img_path . '/' . $default_row['smiley_url'],
						'IMG_PATH'			=> $img_path,
			...
			case 'modify':
				...
				foreach ($images as $image)
				{
					...
					else if (!file_exists($engine_root_path . $img_path . '/' . $image))
					{
					...
					else
					{
						...
						{
							$img_size = getimagesize($engine_root_path . $img_path . '/' . $image);
			...
			case 'import':
				...
				if ($pak != '')
				{
					...
					if (!($pak_ary = @file($engine_root_path . $img_path . '/' . $pak)))
					{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('items', array(
				...
				'ALT_TEXT'		=> $alt_text,
				'IMG_SRC'		=> $engine_root_path . $img_path . '/' . $row[$fields . '_url'],
