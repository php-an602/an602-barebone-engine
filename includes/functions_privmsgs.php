...
function get_folder($user_id, $folder_id = false)
{
	...
	foreach ($folder as $f_id => $folder_ary)
	{
		...
		$template->assign_block_vars('folder', array(
			...
			'U_FOLDER'			=> ($f_id > 0) ? append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $f_id) : append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $folder_id_name),
		...
		!in_array($cur_folder_id, array(PRIVMSGS_NO_BOX, PRIVMSGS_OUTBOX)) && $cur_folder_id != $dest_folder)
	{
		...
		if ($dest_folder != PRIVMSGS_INBOX)
		{
			...
			if ($message_limit && $row['pm_count'] + count($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $row['folder_id']) . '">', '</a>', $row['folder_name']);
		...
		else
		{
			...
			if ($message_limit && $num_messages + count($move_msg_ids) > $message_limit)
			{
				...
				$message .= sprintf($user->lang['CLICK_RETURN_FOLDER'], '<a href="' . append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;folder=inbox') . '">', '</a>', $user->lang['PM_INBOX']);
				trigger_error($message);
			}
		}
...
function update_unread_status($unread, $msg_id, $user_id, $folder_id)
{
	...
	global $db, $user, $engine_container;

	/* @var $engine_notifications \phpbb\notification\manager */
	$engine_notifications = $engine_container->get('notification_manager');

	$engine_notifications->mark_notifications('notification.type.pm', $msg_id, $user_id);
...
{
	global $db, $user, $engine_root_path, $phpEx, $request;
	...
	switch ($mark_action)
	{
		...
		case 'delete_marked':
			...
			if (confirm_box(true))
			{
				...
				$redirect = append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;folder=' . $cur_folder_id);
...
function delete_pm($user_id, $msg_ids, $folder_id)
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_pm_before', compact($vars)));
	...
	$engine_notifications->delete_notifications('notification.type.pm', array_keys($delete_rows));
...
function phpbb_delete_users_pms($user_ids)
{
	...
	if (!empty($undelivered_msg))
	{
		...
		if (!empty($delivered_msg))
		{
			...
			$engine_notifications->delete_notifications('notification.type.pm', $delivered_msg);
		}
		...
		if (!empty($undelivered_msg))
		{
			...
			$engine_notifications->delete_notifications('notification.type.pm', $undelivered_msg);
		}
	}
	...
	if (!empty($delete_ids))
	{
		...
		if (!empty($delete_ids))
		{
			...
			$engine_notifications->delete_notifications('notification.type.pm', $delete_ids);
		}
	}
...
{
	global $db, $user, $template, $engine_root_path, $phpEx, $engine_container;
...
{
	global $db, $auth, $config, $user, $engine_root_path, $engine_container, $engine_dispatcher, $request;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.submit_pm_before', compact($vars)));
	$data_ary = $data;
	unset($data);
	...
	if ($mode == 'edit')
	{
		$engine_notifications->update_notifications('notification.type.pm', $pm_data);
	}
	else
	{
		$engine_notifications->add_notifications('notification.type.pm', $pm_data);
	}
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.submit_pm_after', compact($vars)));
	$data_ary = $data;
	unset($data);
...
{
	global $db, $user, $template, $engine_root_path, $phpEx, $auth, $engine_dispatcher;
	...
	*/
	$vars = array('sql_ary');
	extract($engine_dispatcher->trigger_event('core.message_history_modify_sql_ary', compact($vars)));
	...
	$rowset = array();
	$folder_url = append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm') . '&amp;folder=';
	...
	$url = append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm');
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.message_history_modify_rowset', compact($vars)));
	...
	for ($i = 0, $size = count($rowset); $i < $size; $i++)
	{
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.message_history_modify_template_vars', compact($vars)));
...
{
	global $db, $engine_root_path, $phpEx, $user, $engine_container;
		...
		foreach ($adr_ary as $type => $id_ary)
		{
			foreach ($id_ary as $ug_id => $_id)
			{
				...
				else
				{
					...
					$link = '<a href="' . append_sid("{$engine_root_path}memberlist.$phpEx", 'mode=group&amp;g=' . $ug_id) . '"' . $user_colour . '>';
