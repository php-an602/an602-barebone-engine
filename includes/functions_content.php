...
function gen_sort_selects(&$limit_days, &$sort_by_text, &$sort_days, &$sort_key, &$sort_dir, &$s_limit_days, &$s_sort_key, &$s_sort_dir, &$u_sort_param, $def_st = false, $def_sk = false, $def_sd = false)
{
	...
	extract($engine_dispatcher->trigger_event('core.gen_sort_selects_after', compact($vars)));
...
{
	global $config, $auth, $template, $user, $db, $engine_path_helper, $engine_dispatcher;
	...
	*/
	$vars = array('rowset');
	extract($engine_dispatcher->trigger_event('core.make_jumpbox_modify_forum_list', compact($vars)));
	...
	foreach ($rowset as $row)
	{
		...
		if (!$display_jumpbox)
		{
			$tpl_ary[] = array(
				...
				'LINK'			=> $engine_path_helper->append_url_params($action, array('f' => $forum_id)),
			);
		...
		$tpl_ary[] = array(
			...
			'LINK'			=> $engine_path_helper->append_url_params($action, array('f' => $row['forum_id'])),
		);
		...
		extract($engine_dispatcher->trigger_event('core.make_jumpbox_modify_tpl_ary', compact($vars)));
	...
	$url_parts = $engine_path_helper->get_url_parts($action);
...
function decode_message(&$message, $bbcode_uid = '')
{
	...
	extract($engine_dispatcher->trigger_event('core.decode_message_before', compact($vars)));
	$message = $message_text;

	if (preg_match('#^<[rt][ >]#', $message))
	{
		$message = htmlspecialchars($engine_container->get('text_formatter.utils')->unparse($message), ENT_COMPAT);
	}
	else
	{
		if ($bbcode_uid)
	...
	*/
	$message_text = $message;
	$vars = array('message_text', 'bbcode_uid');
	extract($engine_dispatcher->trigger_event('core.decode_message_after', compact($vars)));
	$message = $message_text;
}
...
function strip_bbcode(&$text, $uid = '')
{
	...
	{
		$text = utf8_htmlspecialchars($engine_container->get('text_formatter.utils')->clean_formatting($text));
	}
	else
	{
		if (!$uid)
...
function generate_text_for_display($text, $uid, $bitfield, $flags, $censor_text = true)
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_display_before', compact($vars)));

	if (preg_match('#^<[rt][ >]#', $text))
	{
		$renderer = $engine_container->get('text_formatter.renderer');
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_display_after', compact($vars)));
...
{
	global $engine_root_path, $phpEx, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_storage_before', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_storage_after', compact($vars)));
...
function generate_text_for_edit($text, $uid, $flags)
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_edit_before', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_text_for_edit_after', compact($vars)));
...
{
	global $config, $user, $engine_path_helper, $engine_dispatcher;
	...
	else
	{
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.smiley_text_root_path', compact($vars)));
...
function parse_attachments($forum_id, &$message, &$attachments, &$update_count_ary, $preview = false)
{
	...
	global $template, $cache, $user, $engine_dispatcher;
	global $extensions, $config, $engine_root_path, $phpEx;
	...
	foreach ($attachments as $attachment)
	{
		...
		$filename = $engine_root_path . $config['upload_path'] . '/' . utf8_basename($attachment['physical_filename']);
		...
		if (isset($extensions[$attachment['extension']]))
		{
			...
			{
				$upload_icon = '<img src="' . $engine_root_path . $config['upload_icons_path'] . '/' . trim($extensions[$attachment['extension']]['upload_icon']) . '" alt="" />';
			}
		}
		...
		if (!$denied)
		{
			...
			$download_link = append_sid("{$engine_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id']);
			...
			switch ($display_cat)
			{
				...
				case ATTACHMENT_CATEGORY_IMAGE:
					$inline_link = append_sid("{$engine_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id']);
				...
				case ATTACHMENT_CATEGORY_THUMB:
					$thumbnail_link = append_sid("{$engine_root_path}download/file.$phpEx", 'id=' . $attachment['attach_id'] . '&amp;t=1');
					$download_link .= '&amp;mode=view';
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.parse_attachments_modify_template_data', compact($vars)));
...
function get_username_string($mode, $user_id, $username, $username_colour = '', $guest_username = false, $custom_profile_url = false)
{
	...
	if (empty($_profile_cache))
	{
		...
		$_profile_cache['base_url'] = append_sid("{$engine_root_path}memberlist.$phpEx", 'mode=viewprofile&amp;u={USER_ID}');
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_username_string', compact($vars)));
...
{
	global $template, $user, $engine_path_helper;

	$lang_string = $user->lang($lang_string);
	$template->assign_block_vars('quickmod', array(
		'VALUE'		=> $option,
		'TITLE'		=> $lang_string,
		'LINK'		=> $engine_path_helper->append_url_params($url, array('action' => $option)),
	));
}
