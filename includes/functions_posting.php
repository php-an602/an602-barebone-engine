...
{
	global $db, $user, $config, $template, $engine_dispatcher, $request;
	global $phpEx, $engine_root_path, $engine_container, $engine_path_helper;

	/* @var $pagination \phpbb\pagination */
	$pagination = $engine_container->get('pagination');
	$base_url = append_sid("{$engine_root_path}posting.$phpEx", 'mode=smilies&amp;f=' . $forum_id);
	...
	if ($mode == 'window')
	{
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.generate_smilies_count_sql_before', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.generate_smilies_modify_sql', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.generate_smilies_modify_rowset', compact($vars)));
	...
	if (count($smilies))
	{
		...
		foreach ($smilies as $row)
		{
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.generate_smilies_before', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.generate_smilies_after', compact($vars)));
...
function update_post_information($type, $ids, $return_update_sql = false)
{
	...
	if (count($last_post_ids))
	{
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.update_post_info_modify_posts_sql', compact($vars)));
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.update_post_info_modify_sql', compact($vars)));
		unset($rowset);
	}
...
{
	global $engine_root_path, $config, $template, $cache;
...
{
	global $config, $engine_filesystem, $engine_dispatcher;
	...
	extract($engine_dispatcher->trigger_event('core.thumbnail_create_before', compact($vars)));
	...
	try
	{
		$engine_filesystem->phpbb_chmod($destination, \phpbb\filesystem\filesystem_interface::CHMOD_READ | \phpbb\filesystem\filesystem_interface::CHMOD_WRITE);
	}
...
{
	global $template, $cache, $config, $engine_root_path, $phpEx, $user, $engine_dispatcher;
	...
	extract($engine_dispatcher->trigger_event('core.modify_default_attachments_template_vars', compact($vars)));
	...
	if (count($attachment_data))
	{
		...
		foreach ($attachment_data as $count => $attach_row)
		{
			...
			$download_link = append_sid("{$engine_root_path}download/file.$phpEx", 'mode=view&amp;id=' . (int) $attach_row['attach_id'], true, ($attach_row['is_orphan']) ? $user->session_id : false);
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.modify_inline_attachments_template_vars', compact($vars)));
...
function load_drafts($topic_id = 0, $forum_id = 0, $id = 0, $pm_action = '', $msg_id = 0)
{
	...
	global $engine_root_path, $engine_dispatcher, $phpEx;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.load_drafts_draft_list_result', compact($vars)));
	...
	foreach ($draft_rows as $draft)
	{
			...
			))
		{
			...
			$link_topic = true;
			$view_url = append_sid("{$engine_root_path}viewtopic.$phpEx", 't=' . $draft['topic_id']);
			$title = $topic_rows[$draft['topic_id']]['topic_title'];

			$insert_url = append_sid("{$engine_root_path}posting.$phpEx", 't=' . $draft['topic_id'] . '&amp;mode=reply&amp;d=' . $draft['draft_id']);
		}
		else if ($draft['forum_id'] && $auth->acl_get('f_read', $draft['forum_id']))
		{
			$link_forum = true;
			$view_url = append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $draft['forum_id']);
			$title = $draft['forum_name'];

			$insert_url = append_sid("{$engine_root_path}posting.$phpEx", 'f=' . $draft['forum_id'] . '&amp;mode=post&amp;d=' . $draft['draft_id']);
		}
		else
		{
			// Either display as PM draft if forum_id and topic_id are empty or if access to the forums has been denied afterwards...
			$link_pm = true;
			$insert_url = append_sid("{$engine_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d={$draft['draft_id']}" . (($pm_action) ? "&amp;action=$pm_action" : '') . (($msg_id) ? "&amp;p=$msg_id" : ''));
		}
...
function topic_review($topic_id, $forum_id, $mode = 'topic_review', $cur_post_id = 0, $show_quote_button = true)
{
	...
	global $config, $engine_root_path, $phpEx, $engine_container, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.topic_review_modify_sql_ary', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.topic_review_modify_post_list', compact($vars)));
	...
	for ($i = 0, $end = count($post_list); $i < $end; ++$i)
	{
		...
		$u_show_post = append_sid($engine_root_path . 'viewtopic.' . $phpEx, "t=$topic_id&amp;p={$row['post_id']}&amp;view=show#p{$row['post_id']}");
		...
		$post_row = array(
			...
			'U_MCP_DETAILS'		=> ($auth->acl_get('m_info', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=main&amp;mode=post_details&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.topic_review_modify_row', compact($vars)));
...
{
	global $db, $user, $engine_container, $engine_dispatcher;
	global $config, $phpEx, $engine_root_path;
	...
	{
		$engine_content_visibility->set_post_visibility(ITEM_DELETED, $post_id, $topic_id, $forum_id, $user->data['user_id'], time(), $softdelete_reason, ($data['topic_first_post_id'] == $post_id), ($data['topic_last_post_id'] == $post_id));
	}
	else if (!$is_soft)
	{
	...
	switch ($post_mode)
	{
		case 'delete_topic':
			...
			if ($is_soft)
			{
				$engine_content_visibility->set_topic_visibility(ITEM_DELETED, $topic_id, $forum_id, $user->data['user_id'], time(), $softdelete_reason);
			}
			else
			{
				delete_topics('topic_id', array($topic_id), false);

				$engine_content_visibility->remove_topic_from_statistic($data, $sql_data);
	...
	if (($post_mode == 'delete') || ($post_mode == 'delete_last_post') || ($post_mode == 'delete_first_post'))
	{
		if (!$is_soft)
		{
			$engine_content_visibility->remove_post_from_statistic($data, $sql_data);
		}
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.delete_post_after', compact($vars)));
...
{
	global $db, $auth, $user, $config, $phpEx, $engine_root_path, $engine_container, $engine_dispatcher, $engine_log, $request;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_submit_post_data', compact($vars)));
	$poll_ary = $poll;
	$data_ary = $data;
	unset($poll);
	unset($data);
	...
	switch ($post_mode)
	{
		...
		case 'edit_topic':
			...
			if ($user->data['user_id'] != $poster_id)
			{
				...
				$engine_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_POST_EDITED', false, array(
					'forum_id' => $data_ary['forum_id'],
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.submit_post_modify_sql_data', compact($vars)));
	$poll_ary = $poll;
	$data_ary = $data;
	unset($poll);
	unset($data);
	...
	if (!empty($data_ary['attachment_data']) && $data_ary['post_id'] && in_array($mode, array('post', 'reply', 'quote', 'edit')))
	{
		...
		foreach ($data_ary['attachment_data'] as $pos => $attach_row)
		{
			...
			else
			{
				...
				if (!@file_exists($engine_root_path . $config['upload_path'] . '/' . utf8_basename($orphan_rows[$attach_row['attach_id']]['physical_filename'])))
				{
					continue;
				}
	...
	if (($post_mode != 'post' && $post_mode != 'reply') && $data_ary['post_visibility'] != $post_visibility)
	{
		...
		/* @var $engine_content_visibility \phpbb\content_visibility */
		$engine_content_visibility = $engine_container->get('content.visibility');
		$engine_content_visibility->set_post_visibility($post_visibility, $data_ary['post_id'], $data_ary['topic_id'], $data_ary['forum_id'], $user->data['user_id'], time(), '', $is_starter, $is_latest);
	}
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.modify_submit_notification_data', compact($vars)));
	...
	if ($post_visibility == ITEM_APPROVED)
	{
		...
		{
			case 'post':
				$engine_notifications->add_notifications(array(
			...
			case 'edit_last_post':
				if ($user->data['user_id'] == $poster_id)
				{
					$engine_notifications->update_notifications(array(
						'notification.type.quote',
					), $notification_data);
				}
	...
	else if ($post_visibility == ITEM_UNAPPROVED)
	{
		...
		{
			case 'post':
				$engine_notifications->add_notifications('notification.type.topic_in_queue', $notification_data);
			break;

			case 'reply':
			case 'quote':
				$engine_notifications->add_notifications('notification.type.post_in_queue', $notification_data);
			break;
	...
	else if ($post_visibility == ITEM_REAPPROVE)
	{
		switch ($mode)
		{
			...
			case 'edit_first_post':
				...
				$engine_notifications->delete_notifications('notification.type.approve_post', $notification_data['post_id']);
			break;
	...
	$params = [];
	$add_anchor = '';
	$url = "{$engine_root_path}viewtopic.$phpEx";
	...
	{
		$url = "{$engine_root_path}viewforum.$phpEx";
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.submit_post_end', compact($vars)));
	$data_ary = $data;
	$poll_ary = $poll;
	unset($data);
	unset($poll);
...
{
	global $config, $db, $user, $phpEx, $engine_root_path, $engine_log;
	...
	$engine_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_BUMP_TOPIC', false, array(
		'forum_id' => $forum_id,
		'topic_id' => $topic_id,
		$post_data['topic_title']
	));

	$url = append_sid("{$engine_root_path}viewtopic.$phpEx", "p={$post_data['topic_last_post_id']}") . "#p{$post_data['topic_last_post_id']}";
...
function phpbb_handle_post_delete($forum_id, $topic_id, $post_id, &$post_data, $is_soft = false, $delete_reason = '')
{
	...
	global $engine_root_path, $phpEx, $engine_log, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.handle_post_delete_conditions', compact($vars)));
	...
	if ($force_delete_allowed || ($is_soft && $force_softdelete_allowed) || $auth->acl_get("m_$perm_check", $forum_id) || ($post_data['poster_id'] == $user->data['user_id'] && $user->data['is_registered'] && $auth->acl_get("f_$perm_check", $forum_id) && $post_id == $post_data['topic_last_post_id'] && !$post_data['post_edit_locked'] && ($post_data['post_time'] > time() - ($config['delete_time'] * 60) || !$config['delete_time'])))
	{
		...
		if (confirm_box(true))
		{
			...
			{
				$engine_log->add('mod', $user->data['user_id'], $user->ip, (($is_soft) ? 'LOG_SOFTDELETE_TOPIC' : 'LOG_DELETE_TOPIC'), false, array(
					'forum_id' => $forum_id,
					'topic_id' => $topic_id,
					$post_data['topic_title'],
					$post_username,
				...
				$meta_info = append_sid("{$engine_root_path}viewforum.$phpEx", "f=$forum_id");
				$message = $user->lang['POST_DELETED'];
			}
			else
			{
				$engine_log->add('mod', $user->data['user_id'], $user->ip, (($is_soft) ? 'LOG_SOFTDELETE_POST' : 'LOG_DELETE_POST'), false, array(
					'forum_id' => $forum_id,
					'topic_id' => $topic_id,
					'post_id'  => $post_id,
				...
				$meta_info = append_sid("{$engine_root_path}viewtopic.$phpEx", "p=$next_post_id") . "#p$next_post_id";
				$message = $user->lang['POST_DELETED'];
			...
			if (!$request->is_ajax())
			{
				$message .= '<br /><br />' . $user->lang('RETURN_FORUM', '<a href="' . append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $forum_id) . '">', '</a>');
			}
