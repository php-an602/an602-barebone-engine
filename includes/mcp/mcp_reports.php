...
class mcp_reports
{
	...
	function main($id, $mode)
	{
		...
		global $config, $engine_root_path, $phpEx, $action, $engine_container, $engine_dispatcher;
		...
		switch ($mode)
		{
			case 'report_details':
				...
				*/
				...
				extract($engine_dispatcher->trigger_event('core.mcp_reports_report_details_query_before', compact($vars)));
				...
				*/
				...
				extract($engine_dispatcher->trigger_event('core.mcp_reports_report_details_query_after', compact($vars)));
				...
				$engine_notifications->mark_notifications('report_post', $post_id, $user->data['user_id']);
				...
				$report_template = array(
					'S_MCP_REPORT'			=> true,
					'S_CLOSE_ACTION'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;p=' . $post_id),
					...
					'U_MCP_REPORTER_NOTES'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $report['user_id']),
					'U_MCP_USER_NOTES'			=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $post_info['user_id']),
					'U_MCP_WARN_REPORTER'		=> ($auth->acl_get('m_warn')) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $report['user_id']) : '',
					'U_MCP_WARN_USER'			=> ($auth->acl_get('m_warn')) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_user&amp;u=' . $post_info['user_id']) : '',
					'U_VIEW_FORUM'				=> append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $post_info['forum_id']),
					'U_VIEW_POST'				=> append_sid("{$engine_root_path}viewtopic.$phpEx", 'p=' . $post_info['post_id'] . '#p' . $post_info['post_id']),
					'U_VIEW_TOPIC'				=> append_sid("{$engine_root_path}viewtopic.$phpEx", 't=' . $post_info['topic_id']),

					'EDIT_IMG'				=> $user->img('icon_post_edit', $user->lang['EDIT_POST']),
					'MINI_POST_IMG'			=> ($post_unread) ? $user->img('icon_post_target_unread', 'UNREAD_POST') : $user->img('icon_post_target', 'POST'),
					'UNAPPROVED_IMG'		=> $user->img('icon_topic_unapproved', $user->lang['POST_UNAPPROVED']),

					'RETURN_REPORTS'			=> sprintf($user->lang['RETURN_REPORTS'], '<a href="' . append_sid("{$engine_root_path}mcp.$phpEx", 'i=reports' . (($post_info['post_reported']) ? '&amp;mode=reports' : '&amp;mode=reports_closed') . '&amp;start=' . $start . '&amp;f=' . $post_info['forum_id']) . '">', '</a>'),
				...
				extract($engine_dispatcher->trigger_event('core.mcp_report_template_data', compact($vars)));
			...
			case 'reports_closed':
				...
				*/
				...
				extract($engine_dispatcher->trigger_event('core.mcp_reports_get_reports_query_before', compact($vars)));
				...
				if (count($report_ids))
				{
					...
					extract($engine_dispatcher->trigger_event('core.mcp_reports_modify_reports_data_sql', compact($vars)));
					...
					while ($row = $db->sql_fetchrow($result))
					{
						$post_row = [
							...
							'U_VIEWPOST'				=> append_sid("{$engine_root_path}viewtopic.$phpEx", 'p=' . $row['post_id']) . '#p' . $row['post_id'],
							'U_VIEW_DETAILS'			=> append_sid("{$engine_root_path}mcp.$phpEx", "i=reports&amp;start=$start&amp;mode=report_details&amp;r={$row['report_id']}"),
						...
						extract($engine_dispatcher->trigger_event('core.mcp_reports_modify_post_row', compact($vars)));
...
{
	global $db, $user, $auth, $engine_log, $request;
	global $phpEx, $engine_root_path, $engine_container;
	...
	if (confirm_box(true))
	{
		...
		foreach ($reports as $report)
		{
			if ($pm)
			{
				$engine_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_PM_REPORT_' .  strtoupper($action) . 'D', false, array(
					'forum_id' => 0,
					'topic_id' => 0,
					$post_info[$report['pm_id']]['message_subject']
				));
				$engine_notifications->delete_notifications('notification.type.report_pm', $report['pm_id']);
			}
			else
			{
				$engine_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_REPORT_' .  strtoupper($action) . 'D', false, array(
				...
				$engine_notifications->delete_notifications('notification.type.report_post', $report['post_id']);
			}
		}
		...
		if (count($notify_reporters))
		{
			foreach ($notify_reporters as $report_id => $reporter)
			{
				...
				if ($pm)
				{
					$engine_notifications->add_notifications('notification.type.report_pm_closed', array_merge($post_info[$post_id], array(
				...
				{
					$engine_notifications->add_notifications('notification.type.report_post_closed', array_merge($post_info[$post_id], array(
						'reporter'			=> $reporter['user_id'],
						'closer_id'			=> $user->data['user_id'],
					)));
				}
	...
	else
	{
		...
		if (!$pm)
		{
			...
			{
				$return_forum = sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . current($forum_ids)) . '">', '</a>') . '<br /><br />';
			}

			if (count($topic_ids) === 1)
			{
				$return_topic = sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$engine_root_path}viewtopic.$phpEx", 't=' . current($topic_ids)) . '">', '</a>') . '<br /><br />';
			}
		}
