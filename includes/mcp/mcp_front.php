...
function mcp_front_view($id, $mode, $action)
{
	...
	if ($module->loaded('queue'))
	{
		...
		if (!empty($forum_list))
		{
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.mcp_front_queue_unapproved_total_before', compact($vars)));
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.mcp_front_view_queue_postid_list_after', compact($vars)));
			...
			if ($total)
			{
				...
				extract($engine_dispatcher->trigger_event('core.mcp_front_view_modify_posts_data_sql', compact($vars)));
				...
				{
					$unapproved_post_row = [
						'U_POST_DETAILS'	=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;p=' . $row['post_id']),
						'U_MCP_FORUM'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=main&amp;mode=forum_view&amp;f=' . $row['forum_id']),
						'U_MCP_TOPIC'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=main&amp;mode=topic_view&amp;t=' . $row['topic_id']),
						'U_FORUM'			=> append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']),
					...
					extract($engine_dispatcher->trigger_event('core.mcp_front_view_modify_unapproved_post_row', compact($vars)));
			...
			$s_hidden_fields = build_hidden_fields(array(
				'redirect'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=main' . (($forum_id) ? '&amp;f=' . $forum_id : ''))
			));

			$template->assign_vars(array(
				'S_HIDDEN_FIELDS'		=> $s_hidden_fields,
				'S_MCP_QUEUE_ACTION'	=> append_sid("{$engine_root_path}mcp.$phpEx", "i=queue"),
	...
	if ($module->loaded('reports'))
	{
		...
		if (!empty($forum_list))
		{
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.mcp_front_reports_count_query_before', compact($vars)));
			...
			if ($total)
			{
				...
				*/
				$vars = array('sql_ary', 'forum_list');
				extract($engine_dispatcher->trigger_event('core.mcp_front_reports_listing_query_before', compact($vars)));
				...
				{
					$reported_post_row = [
						'U_POST_DETAILS'	=> append_sid("{$engine_root_path}mcp.$phpEx", 'p=' . $row['post_id'] . "&amp;i=reports&amp;mode=report_details"),
						'U_MCP_FORUM'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'f=' . $row['forum_id'] . "&amp;i=$id&amp;mode=forum_view"),
						'U_MCP_TOPIC'		=> append_sid("{$engine_root_path}mcp.$phpEx", 't=' . $row['topic_id'] . "&amp;i=$id&amp;mode=topic_view"),
						'U_FORUM'			=> append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $row['forum_id']),
						'U_TOPIC'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", 't=' . $row['topic_id']),
					...
					extract($engine_dispatcher->trigger_event('core.mcp_front_view_modify_reported_post_row', compact($vars)));
		...
		if ($total)
		{
			...
			foreach ($pm_list as $message_id)
			{
				...
				$template->assign_block_vars('pm_report', array(
					'U_PM_DETAILS'	=> append_sid("{$engine_root_path}mcp.$phpEx", 'r=' . $row['report_id'] . "&amp;i=pm_reports&amp;mode=pm_report_details"),
	...
	$template->assign_var('S_MCP_ACTION', append_sid("{$engine_root_path}mcp.$phpEx"));
	make_jumpbox(append_sid("{$engine_root_path}mcp.$phpEx", 'i=main&amp;mode=forum_view'), 0, false, 'm_', true);
}
