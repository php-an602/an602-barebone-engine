...
class mcp_notes
{
		...
		switch ($mode)
		{
			case 'front':
				$template->assign_vars(array(
					...
					'U_POST_ACTION'		=> append_sid("{$engine_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes'),
	...
	*/
	...
	{
		global $config, $engine_log, $request, $engine_root_path, $phpEx;
		global $template, $db, $user, $auth, $engine_container;
		...
		if (($deletemark || $deleteall) && $auth->acl_get('a_clearlogs'))
		{
			...
			if ($where_sql || $deleteall)
			{
				if (check_form_key('mcp_notes'))
				{
					...
					$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_CLEAR_USER', false, array($userrow['username']));
		...
		if ($usernote && $action == 'add_feedback')
		{
			if (check_form_key('mcp_notes'))
			{
				$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_USER_FEEDBACK', false, array($userrow['username']));
