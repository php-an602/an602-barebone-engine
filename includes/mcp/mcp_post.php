...
function mcp_post_details($id, $mode, $action)
{
	...
	switch ($action)
	{
		case 'whois':
			...
			if ($auth->acl_get('m_info', $post_info['forum_id']))
			{
				...
				$template->assign_vars(array(
					'RETURN_POST'	=> sprintf($user->lang['RETURN_POST'], '<a href="' . append_sid("{$engine_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;p=$post_id") . '">', '</a>'),
					'U_RETURN_POST'	=> append_sid("{$engine_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;p=$post_id"),
		...
		default:
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.mcp_post_additional_options', compact($vars)));
	...
	$mcp_post_template_data = array(
		...
		'U_FIND_USERNAME'		=> append_sid("{$engine_root_path}memberlist.$phpEx", 'mode=searchuser&amp;form=mcp_chgposter&amp;field=username&amp;select_single=true'),
		...
		'RETURN_TOPIC'			=> sprintf($user->lang['RETURN_TOPIC'], '<a href="' . append_sid("{$engine_root_path}viewtopic.$phpEx", "p=$post_id") . "#p$post_id\">", '</a>'),
		'RETURN_FORUM'			=> sprintf($user->lang['RETURN_FORUM'], '<a href="' . append_sid("{$engine_root_path}viewforum.$phpEx", "f={$post_info['forum_id']}&amp;start={$start}") . '">', '</a>'),
		...
		'U_WHOIS'				=> ($auth->acl_get('m_info', $post_info['forum_id'])) ? append_sid("{$engine_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$post_info['poster_ip']}") : '',
	);
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.mcp_post_template_data', compact($vars)));
	...
	if ($auth->acl_get('m_info', $post_info['forum_id']))
	{
		...
		if (count($users_ary))
		{
			...
			foreach ($users_ary as $user_id => $user_row)
			{
				$template->assign_block_vars('userrow', array(
					...
					'U_SEARCHPOSTS' => append_sid("{$engine_root_path}search.$phpEx", 'author_id=' . $user_id . '&amp;sr=topics'))
				);
			}
		}
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			$template->assign_block_vars('iprow', array(
				...
				'U_WHOIS'		=> append_sid("{$engine_root_path}mcp.$phpEx", "i=$id&amp;mode=$mode&amp;action=whois&amp;p=$post_id&amp;ip={$row['poster_ip']}"))
			);
		}
		$db->sql_freeresult($result);
...
{
	global $auth, $db, $config, $engine_root_path, $phpEx, $user, $engine_log, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.mcp_change_poster_after', compact($vars)));
	...
	// Now add log entry
	$engine_log->add('mod', $user->data['user_id'], $user->ip, 'LOG_MCP_CHANGE_POSTER', false, array(
