...
{
	global $user, $template, $auth, $db, $engine_container;
	global $engine_root_path, $request, $phpEx, $config, $engine_dispatcher;
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.ucp_pm_view_message_before', compact($vars)));
	...
	{
		$u_pm = append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;mode=compose&amp;u=' . $author_id);
	}

	if ($config['jab_enable'] && $user_info['user_jabber'] && $auth->acl_get('u_sendim'))
	{
		$u_jabber = append_sid("{$engine_root_path}memberlist.$phpEx", 'mode=contact&amp;action=jabber&amp;u=' . $author_id);
	}
	...
	$msg_data = array(
		...
		'U_AUTHOR_POSTS'	=> ($config['load_search'] && $auth->acl_get('u_search')) ? append_sid("{$engine_root_path}search.$phpEx", "author_id=$author_id&amp;sr=posts") : '',
		...
		'U_REPORT'			=> ($config['allow_pm_report']) ? $engine_container->get('controller.helper')->route('phpbb_report_pm_controller', array('id' => $message_row['msg_id'])) : '',
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.ucp_pm_view_messsage', compact($vars)));
	...
	extract($engine_dispatcher->trigger_event('core.ucp_pm_view_message', compact($vars)));
...
function get_user_information($user_id, $user_row)
{
	...
	{
		$user_row['email'] = ($config['board_email_form'] && $config['email_enable']) ? append_sid("{$engine_root_path}memberlist.$phpEx", "mode=email&amp;u=$user_id") : ((($config['board_hide_emails'] && !$auth->acl_get('a_email')) || empty($user_row['user_email'])) ? '' : 'mailto:' . $user_row['user_email']);
	}
