...
class ucp_main
{
	...
	{
		global $config, $db, $user, $auth, $template, $engine_root_path, $phpEx, $engine_dispatcher;
		global $request;
		...
		switch ($mode)
		{
			case 'front':
				...
				if (!empty($forum_ary))
				{
					...
					extract($engine_dispatcher->trigger_event('core.ucp_main_front_modify_sql', compact($vars)));
				...
				foreach ($topic_list as $topic_id)
				{
					...
					$topicrow = array(
						...
						'U_LAST_POST'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", "p=" . $row['topic_last_post_id']) . '#p' . $row['topic_last_post_id'],
						'U_LAST_POST_AUTHOR'	=> get_username_string('profile', $row['topic_last_poster_id'], $row['topic_last_poster_name'], $row['topic_last_poster_colour']),
						'U_NEWEST_POST'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id&amp;view=unread") . '#unread',
						'U_VIEW_TOPIC'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id"),
					);
					...
					extract($engine_dispatcher->trigger_event('core.ucp_main_front_modify_template_vars', compact($vars)));
				...
				if ($config['load_user_activity'])
				{
					...
					{
						include_once($engine_root_path . 'includes/functions_display.' . $phpEx);
					}
...
//					'S_GROUP_OPTIONS'	=> $group_options,
					...
					'U_SEARCH_USER'		=> ($auth->acl_get('u_search')) ? append_sid("{$engine_root_path}search.$phpEx", 'author_id=' . $user->data['user_id'] . '&amp;sr=posts') : '',
				));
			...
			case 'subscribed':
				 ...
				 * @since 3.1.10-RC1
				 */
				$engine_dispatcher->dispatch('core.ucp_main_subscribed_post_data');
				...
				if ($unwatch)
				{
					...
					$message = $msg . '<br /><br />' . sprintf($user->lang['RETURN_UCP'], '<a href="' . append_sid("{$engine_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed") . '">', '</a>');
					meta_refresh(3, append_sid("{$engine_root_path}ucp.$phpEx", "i=$id&amp;mode=subscribed"));
				...
				if ($config['allow_forum_notify'])
				{
					...
					extract($engine_dispatcher->trigger_event('core.ucp_main_subscribed_forums_modify_query', compact($vars)));
					...
					while ($row = $db->sql_fetchrow($result))
					{
						...
						extract($engine_dispatcher->trigger_event('core.ucp_main_subscribed_forum_modify_template_vars', compact($vars)));
			...
			case 'drafts':
				...
				include_once($engine_root_path . 'includes/message_parser.' . $phpEx);
				...
				foreach ($draftrows as $draft)
				{
					...
					{
						$link_pm = true;
						$insert_url = append_sid("{$engine_root_path}ucp.$phpEx", "i=$id&amp;mode=compose&amp;d=" . $draft['draft_id']);
					}
	...
	*/
	...
	{
		global $user, $db, $template, $config, $cache, $auth, $engine_root_path, $phpEx, $engine_container, $request, $engine_dispatcher;
		...
		extract($engine_dispatcher->trigger_event('core.ucp_main_topiclist_count_modify_query', compact($vars)));
		...
		extract($engine_dispatcher->trigger_event('core.ucp_main_topiclist_modify_query', compact($vars)));
		...
		foreach ($topic_list as $topic_id)
		{
			...
			// Replies
			$replies = $engine_content_visibility->get_count('topic_posts', $row, $forum_id) - 1;
			...
			$view_topic_url = append_sid("{$engine_root_path}viewtopic.$phpEx", $view_topic_url_params);
			...
			$template_vars = array(
				...
				'U_NEWEST_POST'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", $view_topic_url_params . '&amp;view=unread') . '#unread',
				'U_LAST_POST'			=> append_sid("{$engine_root_path}viewtopic.$phpEx", $view_topic_url_params . '&amp;p=' . $row['topic_last_post_id']) . '#p' . $row['topic_last_post_id'],
			...
			extract($engine_dispatcher->trigger_event('core.ucp_main_topiclist_topic_modify_template_vars', compact($vars)));

			$template->assign_block_vars('topicrow', $template_vars);

			$pagination->generate_template_pagination(append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id"), 'topicrow.pagination', 'start', $replies + 1, $config['posts_per_page'], 1, true, true);
		}
	}
}
