...
require($engine_root_path . 'includes/functions_user.' . $phpEx);
...
switch ($mode)
{
	...
	case 'logout':
		...
		else if ($user->data['user_id'] != ANONYMOUS)
		{
			...
			$message = $user->lang['LOGOUT_FAILED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$engine_root_path}index.$phpEx") . '">', '</a> ');
	...
	case 'privacy':
		...
		$template->assign_vars(array(
			...
			'U_BACK'				=> append_sid("{$engine_root_path}ucp.$phpEx", 'mode=login'),
		...
		if (confirm_box(true))
		{
			...
			foreach ($request->variable_names(\phpbb\request\request_interface::COOKIE) as $cookie_name)
			{
				...
				*/
				...
				extract($engine_dispatcher->trigger_event('core.ucp_delete_cookies', compact($vars)));
				if ($retain_cookie)
				{
					continue;
				}
			...
			$message = $user->lang['COOKIES_DELETED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$engine_root_path}index.$phpEx") . '">', '</a>');
			trigger_error($message);
		}
		else
		{
	...
	case 'switch_perm':
		...
		$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_ACL_TRANSFER_PERMISSIONS', false, array($user_row['username']));

		$message = sprintf($user->lang['PERMISSIONS_TRANSFERRED'], $user_row['username']) . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$engine_root_path}index.$phpEx") . '">', '</a>');
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.ucp_switch_permissions', compact($vars)));
	...
	case 'restore_perm':
		...
		$engine_log->add('admin', $user->data['user_id'], $user->ip, 'LOG_ACL_RESTORE_PERMISSIONS', false, array($username));

		$message = $user->lang['PERMISSIONS_RESTORED'] . '<br /><br />' . sprintf($user->lang['RETURN_INDEX'], '<a href="' . append_sid("{$engine_root_path}index.$phpEx") . '">', '</a>');
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.ucp_restore_permissions', compact($vars)));
...
if (!$user->data['is_registered'])
{
	...
	{
		$redirect_url = append_sid("{$engine_root_path}ucp.$phpEx?i=pm&p=" . $request->variable('p', 0));
...
if ($module->is_active('zebra', 'friends'))
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.ucp_modify_friends_sql', compact($vars)));
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.ucp_modify_friends_template_vars', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.ucp_display_module_before', compact($vars)));

$template->assign_block_vars('navlinks', array(
	'BREADCRUMB_NAME'	=> $user->lang('UCP'),
	'U_BREADCRUMB'		=> append_sid("{$engine_root_path}ucp.$phpEx"),
));
...
$module->assign_tpl_vars(append_sid("{$engine_root_path}ucp.$phpEx"));
