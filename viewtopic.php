...
if ($view && !$post_id)
{
	...
	if ($view == 'unread')
	{
		...
		$sql = 'SELECT post_id, topic_id, forum_id
			FROM ' . POSTS_TABLE . "
			WHERE topic_id = $topic_id
				AND " . $engine_content_visibility->get_visibility_sql('post', $forum_id) . "
		...
		else
		{
			...
			$sql = 'SELECT topic_id, forum_id
				...
				WHERE forum_id = ' . $forum_id . "
					...
					AND " . $engine_content_visibility->get_visibility_sql('topic', $forum_id) . "
...
if (!$topic_data)
{
	...
	if ($post_id && $topic_id)
	{
		redirect(append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id"));
	}
...
extract($engine_dispatcher->trigger_event('core.viewtopic_modify_forum_id', compact($vars)));
...
if (!$engine_content_visibility->is_visible('topic', $forum_id, $topic_data))
{
	trigger_error('NO_TOPIC');
}
...
if ($post_id)
{
	...
	if ($post_id == $topic_data['topic_first_post_id'] || $post_id == $topic_data['topic_last_post_id'])
	{
		...
		{
			$topic_data['prev_posts'] = $engine_content_visibility->get_count('topic_posts', $topic_data, $forum_id) - 1;
		}
		else
		{
	...
	else
	{
		$sql = 'SELECT COUNT(p.post_id) AS prev_posts
			...
			WHERE p.topic_id = {$topic_data['topic_id']}
				AND " . $engine_content_visibility->get_visibility_sql('post', $forum_id, 'p.');
...
$topic_replies = $engine_content_visibility->get_count('topic_posts', $topic_data, $forum_id) - 1;
...
extract($engine_dispatcher->trigger_event('core.viewtopic_before_f_read_check', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_gen_sort_selects_before', compact($vars)));
...
$viewtopic_url = append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : ''));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_highlight_modify', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_add_quickmod_option_before', compact($vars)));
...
$server_path = (!$view) ? $engine_root_path : generate_board_url() . '/';
...
$base_url = append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id" . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : '') . (($highlight_match) ? "&amp;hilit=$highlight" : ''));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_assign_template_vars_before', compact($vars)));
...
$template->assign_vars(array(
	...
	'U_MCP' 		=> ($auth->acl_get('m_', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", "i=main&amp;mode=topic_view&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : ''), true, $user->session_id) : '',
	...
	'S_TOPIC_ACTION' 		=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start")),
	...
	'S_SEARCHBOX_ACTION'	=> append_sid("{$engine_root_path}search.$phpEx"),
	...
	'U_FORUM'				=> $server_path,
	'U_VIEW_TOPIC' 			=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . (strlen($u_sort_param) ? "&amp;$u_sort_param" : '')),
	'U_CANONICAL'			=> generate_board_url() . '/' . append_sid("viewtopic.$phpEx", "t=$topic_id" . (($start) ? "&amp;start=$start" : ''), true, ''),
	'U_VIEW_FORUM' 			=> append_sid("{$engine_root_path}viewforum.$phpEx", 'f=' . $forum_id),
	'U_VIEW_OLDER_TOPIC'	=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id&amp;view=previous"),
	'U_VIEW_NEWER_TOPIC'	=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id&amp;view=next"),
	'U_PRINT_TOPIC'			=> ($auth->acl_get('f_print', $forum_id)) ? $viewtopic_url . '&amp;view=print' : '',
	'U_EMAIL_TOPIC'			=> ($auth->acl_get('f_email', $forum_id) && $config['email_enable']) ? append_sid("{$engine_root_path}memberlist.$phpEx", "mode=email&amp;t=$topic_id") : '',
	...
	'U_POST_NEW_TOPIC' 		=> ($auth->acl_get('f_post', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$engine_root_path}posting.$phpEx", "mode=post&amp;f=$forum_id") : '',
	'U_POST_REPLY_TOPIC' 	=> ($auth->acl_get('f_reply', $forum_id) || $user->data['user_id'] == ANONYMOUS) ? append_sid("{$engine_root_path}posting.$phpEx", "mode=reply&amp;t=$topic_id") : '',
	'U_BUMP_TOPIC'			=> (bump_topic_allowed($forum_id, $topic_data['topic_bumped'], $topic_data['topic_last_post_time'], $topic_data['topic_poster'], $topic_data['topic_last_poster_id'])) ? append_sid("{$engine_root_path}posting.$phpEx", "mode=bump&amp;t=$topic_id&amp;hash=" . generate_link_hash("topic_$topic_id")) : '')
);
...
if (!empty($topic_data['poll_start']))
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_modify_poll_data', compact($vars)));
	...
	if ($update && $s_can_vote)
	{
		...
		{
			$redirect_url = append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start"));
		...
		if ($request->is_ajax())
		{
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.viewtopic_modify_poll_ajax_data', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_modify_poll_template_data', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_modify_post_list_sql', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_get_post_data', compact($vars)));
...
while ($row = $db->sql_fetchrow($result))
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_post_rowset_data', compact($vars)));
	...
	if (!isset($user_cache[$poster_id]))
	{
		if ($poster_id == ANONYMOUS)
		{
			...
			*/
			...
			extract($engine_dispatcher->trigger_event('core.viewtopic_cache_guest_data', compact($vars)));
		...
		else
		{
			...
			$user_cache_data = array(
				...
				'online'		=> false,
				'jabber'		=> ($config['jab_enable'] && $row['user_jabber'] && $auth->acl_get('u_sendim')) ? append_sid("{$engine_root_path}memberlist.$phpEx", "mode=contact&amp;action=jabber&amp;u=$poster_id") : '',
				'search'		=> ($config['load_search'] && $auth->acl_get('u_search')) ? append_sid("{$engine_root_path}search.$phpEx", "author_id=$poster_id&amp;sr=posts") : '',
			...
			*/
			$vars = array('user_cache_data', 'poster_id', 'row');
			extract($engine_dispatcher->trigger_event('core.viewtopic_cache_user_data', compact($vars)));
			...
			{
				$user_cache[$poster_id]['email'] = ($config['board_email_form'] && $config['email_enable']) ? append_sid("{$engine_root_path}memberlist.$phpEx", "mode=email&amp;u=$poster_id") : (($config['board_hide_emails'] && !$auth->acl_get('a_email')) ? '' : 'mailto:' . $row['user_email']);
			}
			else
			{
...
extract($engine_dispatcher->trigger_event('core.viewtopic_modify_post_data', compact($vars)));
...
for ($i = 0, $end = count($post_list); $i < $end; ++$i)
{
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_modify_post_action_conditions', compact($vars)));
	...
	{
		$u_pm = append_sid("{$engine_root_path}ucp.$phpEx", 'i=pm&amp;mode=compose&amp;action=quotepost&amp;p=' . $row['post_id']);
	}
	...
	$post_row = array(
		...
		'U_EDIT'			=> ($edit_allowed) ? append_sid("{$engine_root_path}posting.$phpEx", "mode=edit&amp;p={$row['post_id']}") : '',
		'U_QUOTE'			=> ($quote_allowed) ? append_sid("{$engine_root_path}posting.$phpEx", "mode=quote&amp;p={$row['post_id']}") : '',
		'U_INFO'			=> ($auth->acl_get('m_info', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", "i=main&amp;mode=post_details&amp;p=" . $row['post_id'], true, $user->session_id) : '',
		'U_DELETE'			=> ($delete_allowed) ? append_sid("{$engine_root_path}posting.$phpEx", 'mode=' . (($softdelete_allowed) ? 'soft_delete' : 'delete') . "&amp;p={$row['post_id']}") : '',
		...
		'U_APPROVE_ACTION'		=> append_sid("{$engine_root_path}mcp.$phpEx", "i=queue&amp;p={$row['post_id']}&amp;redirect=" . urlencode(str_replace('&amp;', '&', $viewtopic_url . '&amp;p=' . $row['post_id'] . '#p' . $row['post_id']))),
		'U_REPORT'			=> ($auth->acl_get('f_report', $forum_id)) ? $engine_container->get('controller.helper')->route('phpbb_report_post_controller', array('id' => $row['post_id'])) : '',
		'U_MCP_REPORT'		=> ($auth->acl_get('m_report', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=reports&amp;mode=report_details&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MCP_APPROVE'		=> ($auth->acl_get('m_approve', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=queue&amp;mode=approve_details&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MCP_RESTORE'		=> ($auth->acl_get('m_approve', $forum_id)) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=queue&amp;mode=' . (($topic_data['topic_visibility'] != ITEM_DELETED) ? 'deleted_posts' : 'deleted_topics') . '&amp;p=' . $row['post_id'], true, $user->session_id) : '',
		'U_MINI_POST'		=> append_sid("{$engine_root_path}viewtopic.$phpEx", 'p=' . $row['post_id']) . '#p' . $row['post_id'],
		'U_MINI_POST_VIEW'	=> append_sid("{$engine_root_path}viewtopic.$phpEx", 'p=' . $row['post_id']) . '&amp;view=show#p' . $row['post_id'],
		'U_NEXT_POST_ID'	=> ($i < $i_total && isset($rowset[$post_list[$i + 1]])) ? $rowset[$post_list[$i + 1]]['post_id'] : '',
		'U_PREV_POST_ID'	=> $prev_post_id,
		'U_NOTES'			=> ($auth->acl_getf_global('m_')) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=notes&amp;mode=user_notes&amp;u=' . $poster_id, true, $user->session_id) : '',
		'U_WARN'			=> ($auth->acl_get('m_warn') && $poster_id != $user->data['user_id'] && $poster_id != ANONYMOUS) ? append_sid("{$engine_root_path}mcp.$phpEx", 'i=warn&amp;mode=warn_post&amp;p=' . $row['post_id'], true, $user->session_id) : '',
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_modify_post_row', compact($vars)));
	...
	*/
	...
	extract($engine_dispatcher->trigger_event('core.viewtopic_post_row_after', compact($vars)));
...
if ($all_marked_read)
{
	...
	{
		$template->assign_vars(array(
			'U_VIEW_UNREAD_POST'	=> append_sid("{$engine_root_path}viewtopic.$phpEx", "t=$topic_id&amp;view=unread") . '#unread',
		));
	}
}
...
if ($s_can_vote || $s_quick_reply)
{
	...
	if ($s_quick_reply)
	{
		...
		$tpl_ary = [
			'S_QUICK_REPLY'			=> true,
			'U_QR_ACTION'			=> append_sid("{$engine_root_path}posting.$phpEx", "mode=reply&amp;t=$topic_id"),
		...
		*/
		...
		extract($engine_dispatcher->trigger_event('core.viewtopic_modify_quick_reply_template_vars', compact($vars)));
...
extract($engine_dispatcher->trigger_event('core.viewtopic_modify_page_title', compact($vars)));
...
make_jumpbox(append_sid("{$engine_root_path}viewforum.$phpEx"), $forum_id);
