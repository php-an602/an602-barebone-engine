...
class upload extends \phpbb\avatar\driver\driver
{
	...
	* @param string $engine_root_path Path to the phpBB root
	...
	*/
	public function __construct(\phpbb\config\config $config, $engine_root_path, $php_ext, \phpbb\filesystem\filesystem_interface $filesystem, \phpbb\path_helper $path_helper, \phpbb\event\dispatcher_interface $dispatcher, \phpbb\files\factory $files_factory, \phpbb\cache\driver\driver_interface $cache = null)
	{
		$this->config = $config;
	...
	*/
	...
	{
		$root_path = (defined('ENGINE_USE_BOARD_URL_PATH') && ENGINE_USE_BOARD_URL_PATH) ? generate_board_url() . '/' : $this->path_helper->get_web_root_path();
